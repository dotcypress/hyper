...
lib.wc.strWidth = function (str) {
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    if (lib.wc.charWidth(codePoint) < 0) {
      return -1;
    }
    rv += (codePoint <= 0xffff) ? 1 : 2;
  }
  return rv;
};
...
hterm.Terminal.IO.prototype.writeUTF8 = function (string) {
  ...
  runes(string).forEach(rune => {
    this.terminal_.getTextAttributes().wcNode = lib.wc.strWidth(rune) > 1;
    this.terminal_.getTextAttributes().unicodeNode = containsNonLatinCodepoints(rune);
    this.terminal_.interpret(rune);
    this.terminal_.getTextAttributes().wcNode = false;
...
hterm.TextAttributes.prototype.isDefault = function () {
  return !this.wcNode && !this.unicodeNode && oldIsDefault.call(this);
};
